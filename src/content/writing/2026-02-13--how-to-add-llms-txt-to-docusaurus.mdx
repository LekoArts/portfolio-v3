...
- **`llms.txt` files only:** Instead of generating `.md` files for each page, it uses the `llms.txt` filename instead. I saw the pattern of providing individual `/llms.txt` files for each page on the [SvelteKit](https://svelte.dev/docs/kit/introduction) site, which I liked a lot — so I copied it. However, it does require [rewrites](#rewrites) so just using the `.md` output is easier.
...
I've added similar rewrite rules to other subroutes of the docs. You should be able to add similar rules to most hosting providers.
