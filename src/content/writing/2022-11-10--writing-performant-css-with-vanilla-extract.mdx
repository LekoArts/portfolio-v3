...
slug: writing-performant-css-with-vanilla-extract
...
![The "Power Rangers" meme with the text: vanilla-extract, dynamic API, recipes, sprinkles, typescript results in CSS styles on a website.](./_img/vanilla-extract-meme.jpg)
...
```tsx title="src/components/app.tsx" {4}
...
```ts title="src/styles/themes.css.ts" collapse={1-55}
import { createThemeContract, createTheme } from "@vanilla-extract/css"
...
```ts title="src/styles/themes.css.ts" collapse={1-93}
import { createThemeContract, createTheme } from "@vanilla-extract/css"
...
```ts title="src/styles/themes.css.ts" collapse={1-93,125-200}
import { createThemeContract, createTheme } from "@vanilla-extract/css"
...
```ts title="src/styles/themes.css.ts" {10, 22}
export const lightThemeClass = createTheme(vars, {
  colors: {
    primary: colors.blue500,
    body: colors.gray700,
...
<Collapsible summary="Pro Tip: Using Open Props">
...
```tsx title="src/components/app.tsx" {2, 9}
import * as React from "react"
...
<Collapsible summary="Why I love styleVariants ❤️">
...
```tsx title="src/components/app.tsx" {5}
import * as React from "react"
import { lightThemeClass, darkThemeClass } from "../styles/themes.css"
import { Stats, StatsItem } from "./stats"
import { useColorScheme } from "../hooks/use-color-scheme"
...
```tsx title="src/components/app.tsx" {3, 4, 8}
return (
  <div className={[schemeClass, styles.wrapper].join(` `)}>
...
> [!NOTE]
> **styleVariants mapping function**
>
...
```tsx title="src/components/app.tsx" {17, 50}
import * as React from "react"
import { lightThemeClass, darkThemeClass } from "../styles/themes.css"
import { Stats, StatsItem } from "./stats"
import { useColorScheme } from "../hooks/use-color-scheme"
...
![Preview of the app screen both in light (top) and dark (bottom) mode. The app has "Last 30 Days" on top left, "Toggle Variant" button on top right. Below that it's a list of different statistics. At the very bottom there is a centred footer.](./_img/style_end.jpg)
...
<Collapsible summary="Tailwind to Sprinkles Comparisons">
...
```ts title="src/styles/sprinkles.css.ts" {12}
// Imports and unresponsiveProperties...
...
<Collapsible summary="Insight: transformBreakpoints">
...
```tsx title="src/components/stats.tsx" {2, 3}
import * as React from "react"
...
![A horizontal top bar with "Last 30 Days" and "Toggle Variant" button. Below that is a list of statistics items which percentages.](./_img/sprinkles_end.jpg)
...
<Collapsible summary="Pro Tip: Your own Box component">
...
> [!WARNING]
> **Things to look out for when combining Sprinkles & Recipes**
>
...
```tsx title="src/components/stats.tsx" {2}
import * as React from "react"
...
```tsx title="src/components/stats.tsx" {8}
// Imports
...
```tsx title="src/components/stats.tsx" {10-13}
// Imports
