...
```js title="plugins/gatsby-source-plausible/gatsby-node.mjs" {3,11-16}
import fetch from "node-fetch"
const POST_NODE_TYPE = `PlausibleTopPage`
...
```js title="plugins/gatsby-source-plausible/gatsby-node.mjs" {18-31}
import fetch from "node-fetch"
const POST_NODE_TYPE = `PlausibleTopPage`
const API_ENDPOINT = `https://plausible.io/api/v1/stats/breakdown`
...
```js title="gatsby-config.mjs" {1,7-13}
