...
```jsx title="src/pages/index.js" collapse={1-3,22-99}
import React from "react"
import { graphql } from "gatsby"
import { GatsbyImage, getImage } from "gatsby-plugin-image"
...
But how does one get the correct values for the three arrays? You can use [SmoothShadows.com](https://smoothshadows.com) to get these values. For my purposes I used 6 layers and only changed the final transparency to `0.15`.
...
```jsx title="src/pages/index.js" {7-9} collapse={1-3,22-99}
import React from "react"
import { graphql } from "gatsby"
import { GatsbyImage, getImage } from "gatsby-plugin-image"
import { rgba } from "polished"
